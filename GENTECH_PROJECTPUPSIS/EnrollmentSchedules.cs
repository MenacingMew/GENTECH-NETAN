        ...
        private void LoadScheduleFromDatabase()
        {
            ...
            try
            {
                using (MySqlConnection conn = DbConnection.GetConnection())
                {
                    ...
                    // Get the student's section from enrollment
                    int studentId = UserSession.EnrollmentStudentID ?? 0;
                 
                    if (studentId == 0)
                    {
                        MessageBox.Show("No student linked to this enrollment credential.\n" +
                                       "Please contact the administrator.",
                                       "No Student Record", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                ...
                SELECT 
                    ...
                    r.Room_Name
                FROM schedule s
                JOIN enrolled_subjects es ON s.Course_ID = es.Course_ID
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                string room = reader["Room_Name"]?.ToString() ?? "";

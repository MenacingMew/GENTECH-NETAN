...
namespace GENTECH_PROJECTPUPSIS
{
    public partial class AdminModifyEnrollment : UserControl
    {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                string programCode = reader["Program_Code"]?.ToString() ?? "N/A";
                                ...
                                if (lblStudentID != null)
                                    lblStudentID.Text = "ID: " + IDFormatter.FormatStudentID(Convert.ToInt32(currentStudentID), DateTime.Now.Year);
                                if (lblSection != null) lblSection.Text = $"{programCode} {yearLevel}-{sectionName}";
                                ...
                                if (lblSection != null) lblSection.Visible = true;
        ...
        private void ClearAll()
        {
            ...
            if (lblSection != null)
            {
                lblSection.Text = "(Course - Year & Section)";

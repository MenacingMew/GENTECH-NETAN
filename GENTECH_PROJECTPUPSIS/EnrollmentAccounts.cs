...
namespace GENTECH_PROJECTPUPSIS
{
    public partial class EnrollmentAccounts : UserControl
    {
        ...
        private async void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            {
                ProcessOverTheCounterPayment(totalAmount);
            }
        }
        ...
        private async void ProcessOverTheCounterPayment(decimal amount)
        {
            string referenceNumber = GenerateOTCReferenceNumber();

            // Record payment with PENDING status - AWAIT this async method
            ...
            {
                // SIMULATION: Verify payment immediately
                btnProceed.Enabled = false;
        ...
        private async Task RecordOverTheCounterPayment(decimal amount, string referenceNumber)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    {
                        MessageBox.Show("No statement of account found.",
                            "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
            ...
            {
                MessageBox.Show($"Error recording OTC payment: {ex.Message}",
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlTransaction transaction = await conn.BeginTransactionAsync())
                    {
                        try
                        {
                            ...
                            if (statementOfAccountId == 0)
                            {
                                MessageBox.Show("No statement of account found for current semester.",
                                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                return false;
                            }
                            ...
                            if (currentBalance <= 0)
                            {
                                MessageBox.Show("No outstanding balance to pay.",
                            ...
                            using (MySqlCommand cmd = new MySqlCommand(insertQuery, conn, transaction))
                            {
                                ...
                                if (result > 0)
                                {
                                    bool updateSuccess = await UpdateStatementOfAccountBalance(conn, transaction,
                                        statementOfAccountId, newBalance);
        ...
        private async Task<bool> UpdateStatementOfAccountBalance(MySqlConnection conn, MySqlTransaction transaction,
            int statementOfAccountId, decimal newBalance)
        {
        ...
        private async Task<bool> ShowGCashQRPayment(decimal amount, string reference)
        {
            ...
            using (QRCodeGenerator qrGenerator = new QRCodeGenerator())
            {
                ...
                using (QRCoderQRCode qrCode = new QRCoderQRCode(qrCodeData))
                {
                    ...
                    {
                        Text = $"Expires: {DateTime.Now.AddMinutes(15):hh:mm tt}",
                    ...
                    Button btnSimulatePaid = new Button()
                    {
                    ...
                    btnSimulatePaid.Click += (sender, e) =>
                    {
                        lblStatus.Text = "✓ Payment received!";
                        ...
                        btnSimulatePaid.Enabled = false;
                        btnCancel.Enabled = false;
                    };

                    qrForm.Controls.AddRange(new Control[] { pbQR, lblInstruction, lblReference, lblExpiry, lblStatus, btnSimulatePaid, btnCancel });
